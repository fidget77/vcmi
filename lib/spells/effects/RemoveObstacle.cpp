...
void RemoveObstacle::apply(const PacketSender * server, RNG & rng, const Mechanics * m, const EffectTarget & target) const
{
	ObstaclesRemoved pack;
	prepareEffects(pack, rng, m, target);
	if(!pack.obstacles.empty())
		server->sendAndApply(&pack);
}

void RemoveObstacle::apply(IBattleState * battleState, RNG & rng, const Mechanics * m, const EffectTarget & target) const
{
	//TODO: evaluate obstacle effects
}
...
std::set<const CObstacleInstance *> RemoveObstacle::getTargets(const Mechanics * m, const EffectTarget & target, bool alwaysMassive) const
{
	...
	{
		for(const auto & obstacle : m->cb->battleGetAllObstacles(BattlePerspective::ALL_KNOWING))
...
void RemoveObstacle::prepareEffects(ObstaclesRemoved & pack, RNG & rng, const Mechanics * m, const EffectTarget & target) const
{
	for(const auto & obstacle : getTargets(m, target, false))
		pack.obstacles.insert(obstacle->uniqueID);
}
